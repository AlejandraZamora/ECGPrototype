...
.controller('MainPageCtrl', ['$rootScope', '$scope', 'persona','personas','$http','$resource', '$location', function ($rootScope, $scope, persona, personas, $http, $resource, $location) {
     ...
     $http.get('/app/ecgData/sample'+numSample+'.txt').success(function (data) {
               var fileToRead=data;
               liness = data.split('\n');
               for(var line = 0; line < liness.length; line++){
                 dataNew.push(liness[line]);
               }
     ...
     var updateData = function(oldData){
        ...
        labels.shift();
        if(count==liness.length){
            readData();
        }
        dataSetA.push(dataNew[count]);
        labels.push(c++);
        count++;
        c=c+0.25;
        dataSetA.shift();    };
...
// ChartJS
   ...
   b.moveTo(g,p);b.lineTo(g,p-d*v(a.datasets[e].data[f],j,k)+c.barStrokeWidth/2);b.lineTo(g+s,p-d*v(a.datasets[e].data[f],j,k)+c.barStrokeWidth/2);b.lineTo(g+s,p);c.barShowStroke&&b.stroke();b.closePath();b.fill()}}},b)},D=window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame||function(a){window.setTimeout(a,1E3/60)},F={}};
    ...
    var cI=0;
    for(var i = 0; i < 125; i++){
         dataI.push(dataNew[i]);
         labelsI.push(cI+0.25);
         cI=cI+0.25;
       }
